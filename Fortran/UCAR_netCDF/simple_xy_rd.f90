...
program simple_xy_rd
  ...
  ! We are reading 2D data, a 6 x 12 grid. But the data array in NetCDF-Fortran is 12 columns 6 rows sliced by (column, row).
  ...
  do x = 1, NX
     do y = 1, NY
        if (data_in(y, x) /= (x - 1) * NY + (y - 1)) then ! We can see data_in(y,x) as "data_in(x,y)", row x column y.
