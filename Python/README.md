...
5. Use `ssh user@host -L 8999:localhost:8999` to re-login into the server, change into a directory (e.g. `mkdir ~/test; cd ~/test`) that you would like to start from, and `conda activate geo; jupyter notebook --NotebookApp.token='' --no-browser --port=8999` to open a Jupyter notebook. Open the url `http://localhost:8999/` within Google-Chrome or other browsers that you like. (**NB** this is for single-hop use. Things are a bit complicated for multiple-hop use and hence are ignored here.)
